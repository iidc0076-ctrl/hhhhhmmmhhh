...
from persistent_interactions import setup_persistent_commands, PersistentInteractionManager

# In your bot setup:
await setup_persistent_commands(bot)

# Optional: Add a background task to monitor and maintain persistent views
async def persistent_maintenance():
    while True:
        try:
            await asyncio.sleep(600)  # Check every 10 minutes
            count = PersistentInteractionManager.get_active_count()
            print(f"Maintaining {count} persistent interactions")
        except Exception as e:
            print(f"Persistent maintenance error: {e}")

# Start maintenance task
asyncio.create_task(persistent_maintenance())
"""