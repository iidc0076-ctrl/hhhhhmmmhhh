...
class UnifiedConfidenceEngine:
    ...
    async def _calculate_enhancement_signals(self, df: pd.DataFrame, indicators: dict, 
        ...
        try:
            ...
            try:
                ...
                try:
                    from enhanced_confluence_analyzer import EnhancedConfluenceAnalyzer
                    analyzer = EnhancedConfluenceAnalyzer()
                    confluence_result = analyzer.analyze_confluence_factors(df, indicators)
                    # Extract signal strength from confluence score
                    confluence_strength = confluence_result.get('confluence_score', 50)
                    enhancement_signals['confluence_factors'] = {
                        'signal': 'NEUTRAL',
                        'strength': min(100, max(0, confluence_strength))
                    }
                except Exception:
    ...
    def _analyze_ict_quick(self, df: pd.DataFrame, indicators: dict) -> dict:
        ...
        try:
            ...
            fvg_signal = 0
            fvg_strength_boost = 0
            if len(closes) >= 3:
                for i in range(max(0, len(closes) - 20), len(closes) - 2):
                    if i + 2 < len(closes):
                        if lows[i] > highs[i+2]:  # Bullish FVG
                            fvg_signal += 1
                            # Check if price is moving into the FVG (bullish bias)
                            if current_price > highs[i+2]:
                                fvg_strength_boost += 3
                        elif highs[i] < lows[i+2]:  # Bearish FVG
                            fvg_signal -= 1
                            # Check if price is moving into the FVG (bearish bias)
                            if current_price < lows[i+2]:
                                fvg_strength_boost += 3
            
            # Determine signal - improved strength calculation
            if fvg_signal > 0:
                strength = min(85, 65 + abs(fvg_signal) * 6 + fvg_strength_boost)
    ...
    def _analyze_ma_trend_quick(self, df: pd.DataFrame, indicators: dict) -> dict:
        ...
        try:
            ...
            # Calculate strength based on alignment with distance bonus
            price_ema_dist = abs(current_price - ema_21) / ema_21 * 100 if ema_21 > 0 else 0
            distance_bonus = min(15, price_ema_dist * 5)  # Bonus for distance from MA
            
            # Calculate strength based on alignment
            if above_sma and above_ema and ema_above_sma:
                return {'signal': 'BUY', 'strength': min(85, 78 + distance_bonus)}
            elif not above_sma and not above_ema and not ema_above_sma:
                return {'signal': 'SELL', 'strength': min(85, 78 + distance_bonus)}
            elif above_ema and ema_above_sma:
                return {'signal': 'BUY', 'strength': min(80, 70 + distance_bonus)}
            elif not above_ema and not ema_above_sma:
                return {'signal': 'SELL', 'strength': min(80, 70 + distance_bonus)}
            else:
                return {'signal': 'NEUTRAL', 'strength': 55}
    ...
    def _analyze_order_flow_quick(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            if buy_ratio > 0.65:
                strength = min(85, 65 + (buy_ratio - 0.5) * 40)  # 65-85% range
                return {'signal': 'BUY', 'strength': strength}
            elif buy_ratio < 0.35:
                sell_ratio = 1 - buy_ratio
                strength = min(85, 65 + (sell_ratio - 0.5) * 40)  # 65-85% range
                return {'signal': 'SELL', 'strength': strength}
            else:
                return {'signal': 'NEUTRAL', 'strength': 55}
    ...
    def _analyze_volume_quick(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            if volume_ratio > 1.5 and recent_close_change > 0:
                strength = min(80, 68 + volume_ratio * 8)  # 68-80% range with volume boost
                return {'signal': 'BUY', 'strength': strength}
    ...
    def _calculate_primary_consensus(self, primary_signals: dict) -> dict:
        ...
        try:
            ...
            for signal_name, signal_data in primary_signals.items():
                # Safety check: ensure signal_data is a dict
                if not isinstance(signal_data, dict):
                    print(f"Warning: signal_data for {signal_name} is not a dict: {type(signal_data)}")
                    signal_data = {'signal': 'NEUTRAL', 'strength': 50}
            ...
            if buy_weight > sell_weight:
                signal = 'BUY'
                # Properly calculate confidence based on weight ratio
                weight_ratio = buy_weight / total_weight  # 0 to 1
                confidence = min(95, 63 + weight_ratio * 32)  # 63 to 95, improved scaling
                strength = weight_ratio * 100
            elif sell_weight > buy_weight:
                signal = 'SELL'
                # Properly calculate confidence based on weight ratio
                weight_ratio = sell_weight / total_weight  # 0 to 1
                confidence = min(95, 63 + weight_ratio * 32)  # 63 to 95, improved scaling
                strength = weight_ratio * 100
            else:
                signal = 'NEUTRAL'
